...
static struct txgbe_stats txgbe_gstrings_stats[] = {
	...
	TXGBE_STAT("rx_long_length_errors", stats.roc),
	TXGBE_STAT("rx_short_length_errors", stats.ruc),
				    ...
				    u64 *data)
{
	...
	for (j = 0; j < TXGBE_NUM_TX_QUEUES; j++) {
	...
	for (j = 0; j < TXGBE_NUM_RX_QUEUES; j++) {
			      ...
			      u8 *data)
{
	...
	case ETH_SS_STATS:
		...
		for (i = 0; i < TXGBE_NUM_TX_QUEUES; i++) {
		...
		for (i = 0; i < TXGBE_NUM_RX_QUEUES; i++) {
...
static struct txgbe_reg_test reg_test_sapphire[] = {
	...
	{ TXGBE_PX_RR_WP(0), 4, PATTERN_TEST, 0x0000FFFF, 0x0000FFFF },
	{ TXGBE_RDB_RFCH(0), 1, PATTERN_TEST, 0x8007FFF0, 0x8007FFF0 },
...
static int txgbe_setup_loopback_test(struct txgbe_adapter *adapter)
{
	struct txgbe_hw *hw = &adapter->hw;
	u32 reg_data;
...
static void txgbe_loopback_cleanup(struct txgbe_adapter *adapter)
{
...
static int txgbe_setup_phy_loopback_test(struct txgbe_adapter *adapter)
{
	...
	u32 value;
	/* setup phy loopback */
	value = rd32_epcs(hw, TXGBE_PHY_MISC_CTL0);
	value |= TXGBE_PHY_MISC_CTL0_TX2RX_LB_EN_0 |
		TXGBE_PHY_MISC_CTL0_TX2RX_LB_EN_3_1;
	wr32_epcs(hw, TXGBE_PHY_MISC_CTL0, value);
			     ...
			     enum ethtool_phys_id_state state)
{
	...
	case ETHTOOL_ID_ON:
		TCALL(hw, mac.ops.led_on, TXGBE_LED_LINK_ACTIVE);
		break;

	case ETHTOOL_ID_OFF:
		TCALL(hw, mac.ops.led_off, TXGBE_LED_LINK_ACTIVE);
		break;
...
static int txgbe_get_rxfh(struct net_device *netdev, u32 *indir, u8 *key)
#else
static int txgbe_get_rxfh(struct net_device *netdev, u32 *indir)
#endif
#endif /* HAVE_RXFH_HASHFUNC */
{
	struct txgbe_adapter *adapter = netdev_priv(netdev);
...
#else
static int txgbe_set_rxfh(struct net_device *netdev, u32 *indir)
#endif
...
static struct ethtool_ops txgbe_ethtool_ops = {
	...
	.get_rxfh_indir_size    = txgbe_rss_indir_size,
	.get_rxfh_indir         = txgbe_get_rxfh,
	.set_rxfh_indir         = txgbe_set_rxfh,
