...
#else
#define TXGBE_SP_MAX_TX_QUEUES  16
#define TXGBE_SP_MAX_RX_QUEUES  16
#endif
...
u32 rd32_epcs(struct txgbe_hw *hw, u32 addr)
{
...
void wr32_ephy(struct txgbe_hw *hw, u32 addr, u32 data)
{
	unsigned int portRegOffset;
...
void wr32_epcs(struct txgbe_hw *hw, u32 addr, u32 data)
{
	unsigned int portRegOffset;
...
s32 txgbe_setup_fc(struct txgbe_hw *hw)
{
	 ...
	 */
	value = rd32_epcs(hw, TXGBE_SR_MII_MMD_AN_ADV);
	value = (value & ~(TXGBE_SR_MII_MMD_AN_ADV_PAUSE_ASM |
		TXGBE_SR_MII_MMD_AN_ADV_PAUSE_SYM)) | pcap;
	wr32_epcs(hw, TXGBE_SR_MII_MMD_AN_ADV, value);
	...
	if (hw->phy.media_type == txgbe_media_type_backplane) {
		value = rd32_epcs(hw, TXGBE_SR_AN_MMD_ADV_REG1);
		value = (value & ~(TXGBE_SR_AN_MMD_ADV_REG1_PAUSE_ASM |
			TXGBE_SR_AN_MMD_ADV_REG1_PAUSE_SYM)) |
			pcap_backplane;
		wr32_epcs(hw, TXGBE_SR_AN_MMD_ADV_REG1, value);
...
s32 txgbe_get_mac_addr(struct txgbe_hw *hw, u8 *mac_addr)
{
	...
	wr32(hw, TXGBE_PSR_MAC_SWC_AD_H, 0x0202);
	wr32(hw, TXGBE_PSR_MAC_SWC_AD_L, 0x03040506);
#endif
...
STATIC s32 txgbe_fc_autoneg_fiber(struct txgbe_hw *hw)
{
	...
	pcs_anadv_reg = rd32_epcs(hw, TXGBE_SR_MII_MMD_AN_ADV);
	pcs_lpab_reg = rd32_epcs(hw, TXGBE_SR_MII_MMD_LP_BABL);
...
STATIC s32 txgbe_fc_autoneg_backplane(struct txgbe_hw *hw)
{
	 ...
	 */
	autoc_reg = rd32_epcs(hw, TXGBE_SR_AN_MMD_ADV_REG1);
	anlp1_reg = rd32_epcs(hw, TXGBE_SR_AN_MMD_LP_ABL1);
				      ...
				      bool *autoneg)
{
	...
	/* SFP */
	else if ((hw->subsystem_id & 0xF0) == TXGBE_ID_SFP) {
		*speed = TXGBE_LINK_SPEED_10GB_FULL;
		*autoneg = false;
	}
	/* XAUI */
	...
	/* KR KX KX4 */
		...
		if (hw->mac.orig_link_settings_stored) {
			...
			sr_pcs_ctl = rd32_epcs(hw, TXGBE_SR_PCS_CTL2);
			sr_pma_mmd_ctl1 = rd32_epcs(hw,
						TXGBE_SR_PMA_MMD_CTL1);
			sr_an_mmd_ctl = rd32_epcs(hw,
						TXGBE_SR_AN_MMD_CTL);
			sr_an_mmd_adv_reg2 = rd32_epcs(hw,
...
s32 txgbe_enable_rx_adapter(struct txgbe_hw *hw)
{
	...
	value = 0;
	while (!(value >> 11)) {
		value = rd32_epcs(hw, TXGBE_PHY_RX_AD_ACK);
		msleep(1);
	}
...
s32 txgbe_set_sgmii_an37_ability(struct txgbe_hw *hw)
{
	...
	wr32_epcs(hw, TXGBE_VR_XS_OR_PCS_MMD_DIGI_CTL1, 0x3002);
	wr32_epcs(hw, TXGBE_SR_MII_MMD_AN_CTL, 0x0105);
	wr32_epcs(hw, TXGBE_SR_MII_MMD_DIGI_CTL, 0x0200);
	value = rd32_epcs(hw, TXGBE_SR_MII_MMD_CTL);
	value = (value & ~0x1200) | (0x1 << 12) | (0x1 << 9);
	wr32_epcs(hw, TXGBE_SR_MII_MMD_CTL, value);
	return 0;
}
...
s32 txgbe_set_link_to_kr(struct txgbe_hw *hw, bool autoneg)
{
	...
	for (i = 0; i < TXGBE_XPCS_POWER_GOOD_MAX_POLLING_TIME; i++) {
		if ((rd32_epcs(hw, TXGBE_VR_XS_OR_PCS_MMD_DIGI_STATUS) &
	...
	if (!autoneg) {
		...
		wr32_epcs(hw, TXGBE_SR_AN_MMD_CTL, 0x0);
		if (hw->revision_id != TXGBE_SP_MPW) {
		...
		wr32_epcs(hw, TXGBE_PHY_MISC_CTL0,
				0xCF00);
		...
		if (i == TXGBE_PHY_INIT_DONE_POLLING_TIME) {
		 ...
		 wr32_epcs(hw, TXGBE_VR_AN_KR_MODE_CL,
				0x1);
	}
out:
	return status;
...
s32 txgbe_set_link_to_kx4(struct txgbe_hw *hw, bool autoneg)
{
	...
	if (!autoneg)
		...
		wr32_epcs(hw, TXGBE_SR_AN_MMD_CTL, 0x3000);

	if (hw->revision_id == TXGBE_SP_MPW) {
		/* Disable PHY MPLLA */
		wr32_ephy(hw, 0x4, 0x2501);
		/* Reset rx lane0-3 clock */
		wr32_ephy(hw, 0x1005, 0x4001);
		wr32_ephy(hw, 0x1105, 0x4001);
		wr32_ephy(hw, 0x1205, 0x4001);
		wr32_ephy(hw, 0x1305, 0x4001);
	} else {
		/* Disable PHY MPLLA for eth mode change(after ECO) */
	...
	wr32_epcs(hw, TXGBE_SR_PCS_CTL2,
			TXGBE_SR_PCS_CTL2_PCS_TYPE_SEL_X);
	/* Set SR PMA MMD Control1 Register Bit[13] = 1'b1  SS13: 10G speed */
	wr32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1,
			TXGBE_SR_PMA_MMD_CTL1_SPEED_SEL_10G);
	...
	wr32_epcs(hw, TXGBE_SR_MII_MMD_DIGI_CTL, 0x0);

	/* 10. Initialize the mode by setting VR XS or PCS MMD Digital Control1
	 * Register Bit[15](VR_RST)
	 */
	wr32_epcs(hw, TXGBE_VR_XS_OR_PCS_MMD_DIGI_CTL1, 0xA000);
	/* wait phy initialization done */
	for (i = 0; i < TXGBE_PHY_INIT_DONE_POLLING_TIME; i++) {
		if ((rd32_epcs(hw, TXGBE_VR_XS_OR_PCS_MMD_DIGI_CTL1) &
			TXGBE_VR_XS_OR_PCS_MMD_DIGI_CTL1_VR_RST) == 0)
			break;
		msleep(100);
	}
			       ...
			       bool autoneg)
{
	...
	if (speed == TXGBE_LINK_SPEED_100_FULL)
		...
		wdata = 0x0100;
	wr32_epcs(hw, TXGBE_SR_MII_MMD_CTL,
			wdata);
	...
	wr32_epcs(hw, TXGBE_SR_MII_MMD_AN_CTL,
			0x0100);
			       ...
			       u32 speed)
{
	...
	if (speed == TXGBE_LINK_SPEED_10GB_FULL) {
		...
		wr32_epcs(hw, TXGBE_SR_PCS_CTL2, 0);
		value = rd32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1);
		value = value | 0x2000;
		wr32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1, value);
		...
		/* @. Set SR PCS Control2 Register Bits[1:0] = 2'b00 PCS_TYPE_SEL: KR */
		wr32_epcs(hw, TXGBE_SR_PCS_CTL2, 0x1);
		/* Set SR PMA MMD Control1 Register Bit[13] = 1'b0  SS13: 1G speed */
		wr32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1, 0x0000);
		/* Set SR MII MMD Control Register to corresponding speed: */
		wr32_epcs(hw, TXGBE_SR_MII_MMD_CTL, 0x0140);
		...
		/* VR MII MMD AN Control Register Bit[8] = 1'b1 MII_CTRL */
		wr32_epcs(hw, TXGBE_SR_MII_MMD_AN_CTL, 0x0100);
	}
	/* 10. Initialize the mode by setting VR XS or PCS MMD Digital Control1
	 * Register Bit[15](VR_RST)
	 */
...
#if 0
	if (txgbe_mng_present(hw)) {
		txgbe_setup_mac_link_hostif(hw, speed);
	} else {
#endif
...
int txgbe_reset_misc(struct txgbe_hw *hw)
{
	...
	value = rd32_epcs(hw, TXGBE_SR_PCS_CTL2);
...
s32 txgbe_reset_hw(struct txgbe_hw *hw)
{
	...
	curr_sr_pcs_ctl = rd32_epcs(hw, TXGBE_SR_PCS_CTL2);
	curr_sr_pma_mmd_ctl1 = rd32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1);
	curr_sr_an_mmd_ctl = rd32_epcs(hw, TXGBE_SR_AN_MMD_CTL);
	curr_sr_an_mmd_adv_reg2 = rd32_epcs(hw,
						TXGBE_SR_AN_MMD_ADV_REG2);
	curr_vr_xs_or_pcs_mmd_digi_ctl1 =
		rd32_epcs(hw, TXGBE_VR_XS_OR_PCS_MMD_DIGI_CTL1);
	 ...
	 */
	sr_pcs_ctl = rd32_epcs(hw, TXGBE_SR_PCS_CTL2);
	sr_pma_mmd_ctl1 = rd32_epcs(hw, TXGBE_SR_PMA_MMD_CTL1);
	sr_an_mmd_ctl = rd32_epcs(hw, TXGBE_SR_AN_MMD_CTL);
	sr_an_mmd_adv_reg2 = rd32_epcs(hw, TXGBE_SR_AN_MMD_ADV_REG2);
