...
#define DRV_VERSION     __stringify(1.1.7)
				...
				struct sk_buff *skb)
{
	...
	if (unlikely(TXGBE_CB(skb)->page_released)) {
		dma_unmap_page(rx_ring->dev, TXGBE_CB(skb)->dma,
			       txgbe_rx_pg_size(rx_ring), DMA_FROM_DEVICE);
		TXGBE_CB(skb)->page_released = false;
	} else {
...
#ifdef SUPPORT_INNER_PROTOCOL_TYPE
				  ...
				  struct txgbe_ring *tx_ring)
{
	...
	if (unlikely(skb_tx(skb)->hardware) &&
	    adapter->ptp_clock &&
	    !test_and_set_bit_lock(__TXGBE_PTP_TX_IN_PROGRESS,
				   &adapter->state)) {
...
static const struct net_device_ops txgbe_netdev_ops = {
	...
	.ndo_change_mtu_rh74    = txgbe_change_mtu,
#else
				...
				const struct pci_device_id __always_unused *ent)
{
	...
	if (adapter->flags & TXGBE_FLAG_FCOE_CAPABLE) {
		unsigned int fcoe_l;
#ifndef SIMULATION_DEBUG
		TCALL(hw, mac.ops.get_device_caps, &device_caps);
		if (!(device_caps & TXGBE_DEVICE_CAPS_FCOE_OFFLOADS)) {
			adapter->flags &= ~TXGBE_FLAG_FCOE_ENABLED;
			adapter->flags &= ~TXGBE_FLAG_FCOE_CAPABLE;
