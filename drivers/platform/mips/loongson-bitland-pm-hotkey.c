...
#define DRIVER_NAME "bitland_pm_hotkey"

#define EC_SCI_DEV  "bitland_ec"
...
static struct quirk_entry quirk_bitland_ls7a_laptop = {
	.sci_irq_num = SCI_IRQ_NUM,
	.is_laptop = 1,
	.is_allinone = 0,
};
...
{
	pr_info("Identified bitland device model '%s'\n", dmi->ident);
...
static const struct dmi_system_id loongson_device_table[] = {
	{
		...
		.matches = {
			...
			DMI_MATCH(DMI_PRODUCT_NAME, "Bitland-LS3A4000-LS7A-laptop"),
			DMI_MATCH(DMI_CHASSIS_TYPE, "9"),
		},
		.callback = dmi_check_cb,
		.driver_data = &quirk_bitland_ls7a_laptop,
	},
	{}
};
...
static struct platform_driver loongson_bitland_pdriver = {
	.probe = loongson_sci_event_probe,
	.driver = {
		.name = "bitland_pm_hotkey",
		.owner = THIS_MODULE,
	},
#ifdef CONFIG_PM
...
void loongson_bitland_sci_hotkey_handler(int event)
{
	int status = 0;
...
static irqreturn_t loongson_sci_int_routine(int irq, void *dev_id)
{
	...
	loongson_bitland_sci_hotkey_handler(event);
	return IRQ_HANDLED;
...
static int sci_pci_init(void)
{
	...
	printk(KERN_INFO "Loongson-bitland: SCI PCI init.\n");

	loongson_sci_device = kmalloc(sizeof(struct sci_device), GFP_KERNEL);
	if (!loongson_sci_device) {
		printk(KERN_ERR "Loongson-bitland: Malloc mem space for sci_dvice failed.\n");
		return -ENOMEM;
	}
	loongson_sci_device->irq = quirks->sci_irq_num;

	printk(KERN_INFO "Loongson-bitland: SCI irq_num:%d.\n", loongson_sci_device->irq);
	...
	if (ret) {
		printk(KERN_ERR "Loongson-bitland: Request irq fail.\n");
		ret = -EFAULT;
		goto out_irq;
	}

	ret = 0;
	printk(KERN_INFO "Loongson-bitland: SCI PCI init successful.\n");
	return ret;
...
static int sci_pci_driver_init(void)
{
	...
	ret = sci_pci_init();
	if (ret) {
		printk(KERN_ERR "Loongson-bitland: SCI Regist pci driver fail.\n");
		return ret;
	}
	printk(KERN_INFO "Loongson-bitland: SCI regist pci driver done.\n");
	return ret;
}

int is_bitland_laptop(void)
{
	if (!dmi_checked) {
...
static int loongson_hotkey_init(void)
{
	...
	loongson_hotkey_dev->name = "Loongson-bitland PM Hotkey Hotkeys";
	...
	if (ret) {
		printk(KERN_ERR "Loongson-bitland PM Hotkey Platform Driver: Fail to setup input device keymap\n");
...
static int loongson_laptop_resume(struct platform_device * pdev)
{
	...
	/* Process LID event */
	loongson_bitland_sci_hotkey_handler(SCI_EVENT_NUM_LID);

	/* Update the power statu when resume */
	if(is_bitland_laptop())
		power_supply_changed(&loongson_ac);
...
{
	int ret = 0;
	printk(KERN_INFO "Loongson-bitland: in probe!\n");
	if (loongson_hotkey_init()) {
		printk(KERN_ERR "Loongson-bitland Platform Driver: Hotkey init fail.\n");
	...
	if (is_bitland_laptop()) {
		...
		if (!power_info) {
			printk(KERN_ERR "Loongson-bitland Platform Driver: Alloc memory for power_info failed!\n");
			ret = -ENOMEM;
		...
		if (power_info->bat_in) {
			...
			printk(KERN_ERR "Loongson-bitland Platform Driver: The battery does not exist!!\n");
		}
	...
	if (ret) {
		printk(KERN_ERR "Loongson-bitland: sci pci driver init fail.\n");
...
fail_backlight_device_register:
	platform_driver_unregister(&loongson_bitland_pdriver);
	return -1;
}

/* Loongson-bitland notebook platfrom init entry */
static int __init loongson_bitland_laptop_init(void)
{
	int ret;

	if (!dmi_check_system(loongson_device_table)) {
		printk(KERN_ERR "Loongson-bitland Laptop not dmimatch devices!\n");
		return -ENODEV;
	}

	dmi_checked = 1;

	printk(KERN_INFO "Loongson-bitland PM Hotkey Platform Driver:regist loongson platform driver begain.\n");

	ret = platform_driver_register(&loongson_bitland_pdriver);
	if (ret) {
		printk(KERN_ERR "Loongson-bitland PM Hotkey Platform Driver: Fail regist loongson platform driver.\n");
		return ret;
	}

	ret = driver_create_file(&loongson_bitland_pdriver.driver, &driver_attr_version);
	if (ret) {
		printk(KERN_ERR "Loongson-bitland creat file fail!!\n");
		return ret;
	}

	printk(KERN_INFO "Loongson-bitland: ret = %d\n", ret);

	platform_device = platform_device_alloc("bitland_pm_hotkey", -1);
	if (!platform_device) {
		ret = -ENOMEM;
...
static void __exit loongson_bitland_laptop_exit(void)
{
	platform_driver_unregister(&loongson_bitland_pdriver);
}

module_init(loongson_bitland_laptop_init);
module_exit(loongson_bitland_laptop_exit);

MODULE_AUTHOR("lichao@loongson.cn");
MODULE_DESCRIPTION("Loongson-bitland PM Hotkey Driver");
MODULE_LICENSE("GPL");
