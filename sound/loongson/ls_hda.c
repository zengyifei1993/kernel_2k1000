...
/* ls: enable rirb interrupt. */
#define EN_RIRBINT 0

/* ls: macros for convenience. */
...
module_param(single_cmd, int, 0444);
...
static void azx_update_rirb(struct azx *chip)
{
	...
	while (chip->rirb.rp != wp) {
		...
		if (res_ex & ICH6_RIRB_EX_UNSOL_EV)
			ls_hda_queue_unsol_event(chip->bus, res, res_ex);
...
static void azx_stream_start(struct azx *chip, struct azx_dev *azx_dev)
{
	...
	/* ls: write byte will flush neighbour bytes to 0. */
...
static void azx_bus_reset(struct hda_bus *bus)
{
	...
	if (chip->initialized) {
		...
		ls_hda_suspend(chip->bus);
		ls_hda_resume(chip->bus);
	}
#endif
	bus->in_reset = 0;
}
...
static int azx_codec_create(struct azx *chip, const char *model)
{
	...
	err = ls_hda_bus_new(chip->card, &bus_temp, &chip->bus);
	if (err < 0)
		return err;
...
static int azx_pcm_open(struct snd_pcm_substream *substream)
{
	...
	snd_pcm_hw_constraint_step(runtime, 0, SNDRV_PCM_HW_PARAM_PERIOD_BYTES,
				   128);
	ls_hda_power_up(apcm->codec);
	err = hinfo->ops.open(hinfo, apcm->codec, substream);
	if (err < 0) {
		azx_release_device(azx_dev);
		ls_hda_power_down(apcm->codec);
		mutex_unlock(&chip->open_mutex);
		return err;
	}
	snd_pcm_limit_hw_rates(runtime);
...
static int azx_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	ls_hda_codec_cleanup(apcm->codec, hinfo, substream);
...
static int azx_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	format_val = ls_hda_calc_stream_format(runtime->rate,
						 runtime->channels,
						 runtime->format,
						 hinfo->maxbps,
	...
	return ls_hda_codec_prepare(apcm->codec, hinfo, stream_tag,
...
/*
 * fix ls pos not reset. when stream clear, dma reset.
...
/* ls: not implemented */
	...
	/* ls: check bogus (too early) interrupt. */
...
{
	return ls_hda_build_controls(chip->bus);
}
...
static int ls_hda_codecs_inuse(struct hda_bus *bus)
{
	struct hda_codec *codec;

	list_for_each_entry(codec, &bus->codec_list, list) {
		if (ls_hda_codec_needs_resume(codec))
			return 1;
	}
	return 0;
}
...
static int azx_resume(struct platform_device *dev)
{
	...
	if (ls_hda_codecs_inuse(chip->bus))
		azx_init_chip(chip, 1);
...
static int azx_halt(struct notifier_block *nb, unsigned long event, void *buf)
{
	...
	ls_hda_bus_reboot_notify(chip->bus);
	azx_stop_chip(chip);
	return NOTIFY_OK;
}
				...
				int dev, int driver_type, struct azx **rchip)
{
	...
	*rchip = NULL;
	...
	chip->msi = 0;		/* ls: disable MSI for non-pci devices */
	chip->dev_index = dev;
	...
	/* ls: get addr for platform driver. */
...
static int azx_probe(struct platform_device *devptr)
{
	...
	/* set this here since it's referred in ls_hda_load_patch() */
	...
	/* create PCM streams */
	err = ls_hda_build_pcms(chip->bus);
	if (err < 0)
		goto out_free;
