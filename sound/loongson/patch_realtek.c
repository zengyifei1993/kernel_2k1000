...
#ifdef CONFIG_LS7A_HDA_POWER_SAVE
			     ...
			     struct snd_ctl_elem_info *uinfo)
{
	...
	if (!spec->input_mux[mux_idx].num_items && mux_idx > 0)
		mux_idx = 0;
	return ls_hda_input_mux_info(&spec->input_mux[mux_idx], uinfo);
}
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (type == AC_WID_AUD_MIX) {
		...
		for (i = 0; i < imux->num_items; i++) {
			...
			ls_hda_codec_amp_stereo(codec, nid, HDA_INPUT,
						 imux->items[i].index,
						 HDA_AMP_MUTE, v);
		}
		*cur_val = idx;
		...
		return ls_hda_input_mux_put(codec, imux, ucontrol, nid,
			    ...
			    struct snd_ctl_elem_info *uinfo)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	return ls_hda_ch_mode_info(codec, uinfo, spec->channel_mode,
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	return ls_hda_ch_mode_get(codec, ucontrol, spec->channel_mode,
				   spec->num_channel_mode,
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct alc_spec *spec = codec->spec;
	int err = ls_hda_ch_mode_put(codec, ucontrol, spec->channel_mode,
				      spec->num_channel_mode,
				      &spec->ext_channel_count);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int pinctl = ls_hda_codec_read(codec, nid, 0,
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (change) {
		...
		ls_hda_codec_write_cache(codec, nid, 0,
		 ...
		 */
		if (val <= 2) {
			ls_hda_codec_amp_stereo(codec, nid, HDA_OUTPUT, 0,
						 HDA_AMP_MUTE, HDA_AMP_MUTE);
			ls_hda_codec_amp_stereo(codec, nid, HDA_INPUT, 0,
						 HDA_AMP_MUTE, 0);
		} else {
			ls_hda_codec_amp_stereo(codec, nid, HDA_INPUT, 0,
						 HDA_AMP_MUTE, HDA_AMP_MUTE);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	long *valp = ucontrol->value.integer.value;
	unsigned int val = ls_hda_codec_read(codec, nid, 0,
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned char mask = (kcontrol->private_value >> 16) & 0xff;
	long val = *ucontrol->value.integer.value;
	unsigned int gpio_data = ls_hda_codec_read(codec, nid, 0,
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned char mask = (kcontrol->private_value >> 16) & 0xff;
	long val = *ucontrol->value.integer.value;
	unsigned int ctrl_data = ls_hda_codec_read(codec, nid, 0,
	...
	if (val==0)
		...
		ctrl_data |= mask;
	ls_hda_codec_write_cache(codec, nid, 0, AC_VERB_SET_DIGI_CONVERT_1,
				  ctrl_data);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!val)
		...
		ctrl_data |= mask;
	ls_hda_codec_write_cache(codec, nid, 0, AC_VERB_SET_EAPD_BTLENABLE,
				  ctrl_data);
			      ...
			      int auto_pin_type)
{
	...
	if (auto_pin_type == AUTO_PIN_MIC) {
		unsigned int pincap;
		unsigned int oldval;
		oldval = ls_hda_codec_read(codec, nid, 0,
					    AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
		pincap = ls_hda_query_pin_caps(codec, nid);
	...
	ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_PIN_WIDGET_CONTROL, val);
}
...
static void alc_fix_pll(struct hda_codec *codec)
{
	...
	if (!spec->pll_nid)
		return;
	ls_hda_codec_write(codec, spec->pll_nid, 0, AC_VERB_SET_COEF_INDEX,
			    spec->pll_coef_idx);
	val = ls_hda_codec_read(codec, spec->pll_nid, 0,
				 AC_VERB_GET_PROC_COEF, 0);
	ls_hda_codec_write(codec, spec->pll_nid, 0, AC_VERB_SET_COEF_INDEX,
			    spec->pll_coef_idx);
	ls_hda_codec_write(codec, spec->pll_nid, 0, AC_VERB_SET_PROC_COEF,
...
#ifdef CONFIG_LS7A_HDA_INPUT_JACK
		...
		hda_nid_t nid, int type)
{
	...
	spec = codec->spec;
	lsda_array_init(&spec->jacks, sizeof(*jack), 32);
	jack = lsda_array_new(&spec->jacks);
	if (!jack)
		return -ENOMEM;
	...
	if (jacks) {
		...
		for (i = 0; i < spec->jacks.used; i++) {
			if (jacks->nid == nid) {
				...
				present = ls_hda_jack_detect(codec, nid);
...
static void alc_automute_speaker(struct hda_codec *codec, int pinctl)
{
	...
	for (i = 0; i < ARRAY_SIZE(spec->autocfg.hp_pins); i++) {
		...
		if (!nid)
			break;
		if (ls_hda_jack_detect(codec, nid)) {
			spec->jack_present = 1;
			break;
		}
	...
	for (i = 0; i < ARRAY_SIZE(spec->autocfg.speaker_pins); i++) {
		...
		if (!nid)
			break;
		if (pinctl) {
			ls_hda_codec_write(codec, nid, 0,
				    AC_VERB_SET_PIN_WIDGET_CONTROL,
				...
				hda_nid_t nid)
{
	...
	nums = ls_hda_get_connections(codec, mux, conn, ARRAY_SIZE(conn));
	for (i = 0; i < nums; i++)
		if (conn[i] == nid)
			return i;
	return -1;
...
static void alc_dual_mic_adc_auto_switch(struct hda_codec *codec)
{
	...
	if (spec->cur_adc && spec->cur_adc != new_adc) {
		...
		__ls_hda_codec_cleanup_stream(codec, spec->cur_adc, 1);
		spec->cur_adc = new_adc;
		ls_hda_codec_setup_stream(codec, new_adc,
...
static void alc_mic_automute(struct hda_codec *codec)
{
	...
	if (type == AC_WID_AUD_MIX) {
		...
		/* MUX style (e.g. ALC880) */
		ls_hda_codec_write_cache(codec, cap_nid, 0,
...
static void alc888_coef_init(struct hda_codec *codec)
{
	...
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_COEF_INDEX, 0);
	tmp = ls_hda_codec_read(codec, 0x20, 0, AC_VERB_GET_PROC_COEF, 0);
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_COEF_INDEX, 7);
	if ((tmp & 0xf0) == 0x20)
		/* alc888S-VC */
		ls_hda_codec_read(codec, 0x20, 0,
...
static void alc889_coef_init(struct hda_codec *codec)
{
	...
	tmp = ls_hda_codec_read(codec, 0x20, 0, AC_VERB_GET_PROC_COEF, 0);
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_COEF_INDEX, 7);
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_PROC_COEF, tmp|0x2010);
}
...
static void set_eapd(struct hda_codec *codec, hda_nid_t nid, int on)
{
	if (get_wcaps_type(get_wcaps(codec, nid)) != AC_WID_PIN)
		return;
	if (ls_hda_query_pin_caps(codec, nid) & AC_PINCAP_EAPD)
		ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_EAPD_BTLENABLE,
				    on ? 2 : 0);
}
...
static void alc_auto_init_amp(struct hda_codec *codec, int type)
{
	...
	switch (type) {
	case ALC_INIT_GPIO1:
		ls_hda_sequence_write(codec, alc_gpio1_init_verbs);
		break;
	case ALC_INIT_GPIO2:
		ls_hda_sequence_write(codec, alc_gpio2_init_verbs);
		break;
	case ALC_INIT_GPIO3:
		ls_hda_sequence_write(codec, alc_gpio3_init_verbs);
		break;
	case ALC_INIT_DEFAULT:
		...
		switch (codec->vendor_id) {
		case 0x10ec0260:
			ls_hda_codec_write(codec, 0x1a, 0,
					    AC_VERB_SET_COEF_INDEX, 7);
			tmp = ls_hda_codec_read(codec, 0x1a, 0,
						 AC_VERB_GET_PROC_COEF, 0);
			ls_hda_codec_write(codec, 0x1a, 0,
					    AC_VERB_SET_COEF_INDEX, 7);
			ls_hda_codec_write(codec, 0x1a, 0,
...
static void alc_init_auto_hp(struct hda_codec *codec)
{
	...
	for (i = 0; i < cfg->hp_outs; i++) {
		snd_printdd("realtek: Enable HP auto-muting on NID 0x%x\n",
			    cfg->hp_pins[i]);
		ls_hda_codec_write_cache(codec, cfg->hp_pins[i], 0,
...
static void alc_init_auto_mic(struct hda_codec *codec)
{
	...
	for (i = 0; i < cfg->num_inputs; i++) {
		...
		unsigned int defcfg;
		defcfg = ls_hda_codec_get_pincfg(codec, nid);
		switch (ls_hda_get_input_pin_attr(defcfg)) {
	...
	spec->auto_mic = 1;
	ls_hda_codec_write_cache(codec, spec->ext_mic.pin, 0,
				  AC_VERB_SET_UNSOLICITED_ENABLE,
...
static int alc_auto_parse_customize_define(struct hda_codec *codec)
{
    ...
    /* TODO: change to platform driver type. by ls */
	ass = codec->subsystem_id & 0xffff;
	nid = 0x1d;
	if (codec->vendor_id == 0x10ec0260)
		nid = 0x17;
	ass = ls_hda_codec_get_pincfg(codec, nid);
...
#endif
		for (; cfg->nid; cfg++)
			ls_hda_codec_set_pincfg(codec, cfg->nid, cfg->val);
	}
...
{
	unsigned int val;
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_COEF_INDEX,
				coef_idx);
	val = ls_hda_codec_read(codec, 0x20, 0,
				AC_VERB_GET_PROC_COEF, 0);
	return val;
}
							...
							unsigned int coef_val)
{
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_COEF_INDEX,
			    coef_idx);
	ls_hda_codec_write(codec, 0x20, 0, AC_VERB_SET_PROC_COEF,
			    coef_val);
}
...
static void alc_auto_init_digital(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->autocfg.dig_outs; i++) {
		...
		if (pin) {
			ls_hda_codec_write(codec, pin, 0,
					    AC_VERB_SET_PIN_WIDGET_CONTROL,
					    PIN_OUT);
		}
	}
...
static void alc_auto_parse_digital(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->autocfg.dig_outs; i++) {
		err = ls_hda_get_connections(codec,
			    ...
			    struct snd_ctl_elem_info *uinfo)
{
	...
	kcontrol->private_value = HDA_COMPOSE_AMP_VAL(spec->adc_nids[0], 3, 0,
						      HDA_INPUT);
	err = ls_hda_mixer_amp_volume_info(kcontrol, uinfo);
			   ...
			   unsigned int size, unsigned int __user *tlv)
{
	...
	kcontrol->private_value = HDA_COMPOSE_AMP_VAL(spec->adc_nids[0], 3, 0,
						      HDA_INPUT);
	err = ls_hda_mixer_amp_tlv(kcontrol, op_flag, size, tlv);
	mutex_unlock(&codec->control_mutex);
	return err;
}
...
static int alc_build_controls(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->num_mixers; i++) {
		err = ls_hda_add_new_ctls(codec, spec->mixers[i]);
		if (err < 0)
			return err;
	}
	if (spec->cap_mixer) {
		err = ls_hda_add_new_ctls(codec, spec->cap_mixer);
		if (err < 0)
			return err;
	}
	if (spec->multiout.dig_out_nid) {
		err = ls_hda_create_spdif_out_ctls(codec,
						    spec->multiout.dig_out_nid);
		if (err < 0)
			return err;
		if (!spec->no_analog) {
			err = ls_hda_create_spdif_share_sw(codec,
							    &spec->multiout);
			if (err < 0)
				return err;
	...
	if (spec->dig_in_nid) {
		err = ls_hda_create_spdif_in_ctls(codec, spec->dig_in_nid);
		if (err < 0)
			return err;
	}
	...
	if (spec->beep_amp) {
		...
		for (knew = alc_beep_mixer; knew->name; knew++) {
			...
			err = ls_hda_ctl_add(codec, 0, kctl);
			if (err < 0)
				return err;
		}
	}
	...
	if (!spec->no_analog &&
	    !ls_hda_find_mixer_ctl(codec, "Master Playback Volume")) {
		unsigned int vmaster_tlv[4];
		ls_hda_set_vmaster_tlv(codec, spec->vmaster_nid,
					HDA_OUTPUT, vmaster_tlv);
		err = ls_hda_add_vmaster(codec, "Master Playback Volume",
					  vmaster_tlv, alc_slave_vols);
		if (err < 0)
			return err;
	}
	if (!spec->no_analog &&
	    !ls_hda_find_mixer_ctl(codec, "Master Playback Switch")) {
		err = ls_hda_add_vmaster(codec, "Master Playback Switch",
					  NULL, alc_slave_sws);
		if (err < 0)
			return err;
	}
	...
	if (spec->capsrc_nids || spec->adc_nids) {
		kctl = ls_hda_find_mixer_ctl(codec, "Capture Source");
		if (!kctl)
			kctl = ls_hda_find_mixer_ctl(codec, "Input Source");
		for (i = 0; kctl && i < kctl->count; i++) {
			hda_nid_t *nids = spec->capsrc_nids;
			if (!nids)
				nids = spec->adc_nids;
			err = ls_hda_add_nid(codec, kctl, i, nids[i]);
			if (err < 0)
				return err;
		}
	}
	if (spec->cap_mixer) {
		...
		for (knew = spec->cap_mixer; knew->name; knew++) {
			if (kname && strcmp(knew->name, kname) == 0)
				continue;
			kctl = ls_hda_find_mixer_ctl(codec, knew->name);
			for (i = 0; kctl && i < kctl->count; i++) {
				err = ls_hda_add_nid(codec, kctl, i,
						      spec->adc_nids[i]);
				if (err < 0)
					return err;
			}
	...
	for (i = 0; i < spec->num_mixers; i++) {
		for (knew = spec->mixers[i]; knew->name; knew++) {
			...
			for (j = 0; j < 4; j++, u >>= 8) {
				...
				err = ls_hda_add_nid(codec, kctl, 0, nid);
				if (err < 0)
					return err;
			}
...
static void alc880_uniwill_mic_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x18);
	bits = present ? HDA_AMP_MUTE : 0;
	ls_hda_codec_amp_stereo(codec, 0x0b, HDA_INPUT, 1, HDA_AMP_MUTE, bits);
}
...
static void alc880_uniwill_p53_dcvol_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_codec_read(codec, 0x21, 0,
				     AC_VERB_GET_VOLUME_KNOB_CONTROL, 0);
	present &= HDA_AMP_VOLMASK;
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_OUTPUT, 0,
				 HDA_AMP_VOLMASK, present);
	ls_hda_codec_amp_stereo(codec, 0x0d, HDA_OUTPUT, 0,
				 HDA_AMP_VOLMASK, present);
}
...
static void alc880_medion_rim_automute(struct hda_codec *codec)
{
	...
	if (spec->jack_present)
		ls_hda_codec_write(codec, 0x01, 0, AC_VERB_SET_GPIO_DATA, 0);
	else
		ls_hda_codec_write(codec, 0x01, 0, AC_VERB_SET_GPIO_DATA, 2);
}
...
static int alc_init(struct hda_codec *codec)
{
	...
	for (i = 0; i < spec->num_init_verbs; i++)
		ls_hda_sequence_write(codec, spec->init_verbs[i]);
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_check_amp_list_power(codec, &spec->loopback, nid);
}
#endif
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_analog_open(codec, &spec->multiout, substream,
					     hinfo);
}
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_analog_prepare(codec, &spec->multiout,
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_analog_cleanup(codec, &spec->multiout);
}
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_dig_open(codec, &spec->multiout);
}
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_dig_prepare(codec, &spec->multiout,
					     stream_tag, format, substream);
}
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_dig_cleanup(codec, &spec->multiout);
}
...
{
	struct alc_spec *spec = codec->spec;
	return ls_hda_multi_out_dig_close(codec, &spec->multiout);
}
				      ...
				      struct snd_pcm_substream *substream)
{
	...
	ls_hda_codec_setup_stream(codec, spec->adc_nids[substream->number + 1],
				   stream_tag, 0, format);
	return 0;
}
				      ...
				      struct snd_pcm_substream *substream)
{
	...
	ls_hda_codec_cleanup_stream(codec,
				       ...
				       struct snd_pcm_substream *substream)
{
	...
	ls_hda_codec_setup_stream(codec, spec->cur_adc, stream_tag, 0, format);
	return 0;
}
...
{
	struct alc_spec *spec = codec->spec;
	ls_hda_codec_cleanup_stream(codec, spec->cur_adc);
	spec->cur_adc = 0;
	return 0;
}
...
static void alc_free(struct hda_codec *codec)
{
	...
	kfree(spec);
	ls_hda_detach_beep_device(codec);
}
...
#ifdef LS7A_HDA_NEEDS_RESUME
static int alc_resume(struct hda_codec *codec)
{
	codec->patch_ops.init(codec);
	ls_hda_codec_resume_amp(codec);
	ls_hda_codec_resume_cache(codec);
	hda_call_check_power_status(codec, 0x01);
	return 0;
}
#endif
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pin_ctl = ls_hda_codec_read(codec, nid, 0,
				     AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	old_ctl = ls_hda_codec_read(codec, nid, 0,
				     AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	sel = ls_hda_codec_read(codec, nid, 0, AC_VERB_GET_CONNECT_SEL, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	sel = ls_hda_codec_read(codec, nid, 0, AC_VERB_GET_CONNECT_SEL, 0) & 3;
		       ...
		       int cidx, unsigned long val)
{
	...
	lsda_array_init(&spec->kctls, sizeof(*knew), 32);
	knew = lsda_array_new(&spec->kctls);
	if (!knew)
		return -ENOMEM;
...
{
	unsigned int pincap = ls_hda_query_pin_caps(codec, nid);
				      ...
				      hda_nid_t cap1, hda_nid_t cap2)
{
	...
	for (i = 0; i < cfg->num_inputs; i++) {
		...
		if (i > 0 && type == cfg->inputs[i - 1].type)
			...
			type_idx = 0;
		label = ls_hda_get_autocfg_input_label(codec, cfg, i);
		if (mixer) {
		...
		if (idx >= 0)
			ls_hda_add_imux_item(imux, label, idx, NULL);
	}
	return 0;
}
...
{
	ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    pin_type);
	/* unmute pin */
	ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_AMP_GAIN_MUTE,
			    AMP_OUT_UNMUTE);
}
					      ...
					      int dac_idx)
{
	...
	if (alc880_is_multi_pin(nid)) {
		...
		ls_hda_codec_write(codec, alc880_idx_to_selector(idx), 0,
				    AC_VERB_SET_CONNECT_SEL,
...
static void alc880_auto_init_input_src(struct hda_codec *codec)
{
	...
	for (c = 0; c < spec->num_adc_nids; c++) {
		...
		if (!imux->num_items && mux_idx > 0)
			imux = &spec->input_mux[0];
		if (imux)
			ls_hda_codec_write(codec, spec->adc_nids[c], 0,
					    AC_VERB_SET_CONNECT_SEL,
					    imux->items[0].index);
	}
}
...
static int alc880_parse_auto_config(struct hda_codec *codec)
{
	...
	err = ls_hda_parse_pin_def_config(codec, &spec->autocfg,
					   alc880_ignore);
	if (err < 0)
		return err;
				    ...
				    int idx)
{
	if (get_wcaps_type(get_wcaps(codec, cap)) == AC_WID_AUD_MIX) {
		ls_hda_codec_amp_stereo(codec, cap, HDA_INPUT, idx,
					 HDA_AMP_MUTE, 0);
	} else {
		ls_hda_codec_write_cache(codec, cap, 0,
				 ...
				 int num_nids)
{
	...
	for (n = 0; n < num_nids; n++) {
		...
		if (get_wcaps_type(get_wcaps(codec, adc)) != AC_WID_AUD_IN)
			continue;
		cap = adc;
		nconns = ls_hda_get_connections(codec, cap, conn,
						 ARRAY_SIZE(conn));
		if (nconns == 1) {
			cap = conn[0];
			nconns = ls_hda_get_connections(codec, cap, conn,
...
static int patch_alc880(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC880_MODEL_LAST,
						  alc880_models,
						  alc880_cfg_tbl);
	if (board_config < 0) {
	...
	err = ls_hda_attach_beep_device(codec, 0x1);
	if (err < 0) {
		alc_free(codec);
		return err;
	}
				    ...
				    hda_nid_t mono)
{
	...
	ls_hda_codec_write(codec, hp, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
	ls_hda_codec_write(codec, line, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
	/* mono (speaker) depending on the HP jack sense */
	val = (val && !spec->jack_present) ? PIN_OUT : 0;
	ls_hda_codec_write(codec, mono, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    val);
}
...
static void alc260_hp_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls_hda_jack_detect(codec, 0x10);
...
static void alc260_hp_3013_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls_hda_jack_detect(codec, 0x15);
...
{
	unsigned int bits = ls_hda_jack_detect(codec, 0x10) ? 0 : PIN_OUT;

	ls_hda_codec_write(codec, 0x0f, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
	ls_hda_codec_write(codec, 0x11, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
	ls_hda_codec_write(codec, 0x15, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    bits);
}
...
static void alc260_replacer_672v_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x0f);
	if (present) {
		ls_hda_codec_write_cache(codec, 0x01, 0,
					  AC_VERB_SET_GPIO_DATA, 1);
		ls_hda_codec_write_cache(codec, 0x0f, 0,
					  AC_VERB_SET_PIN_WIDGET_CONTROL,
					  PIN_HP);
	} else {
		ls_hda_codec_write_cache(codec, 0x01, 0,
					      ...
					      int sel_idx)
{
	...
	if (nid >= 0x12) {
		int idx = nid - 0x12;
		ls_hda_codec_write(codec, idx + 0x0b, 0,
...
static int patch_alc260(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC260_MODEL_LAST,
						  alc260_models,
						  alc260_cfg_tbl);
	if (board_config < 0) {
...
static void alc882_targa_automute(struct hda_codec *codec)
{
	struct alc_spec *spec = codec->spec;
	alc_automute_amp(codec);
	ls_hda_codec_write_cache(codec, 1, 0, AC_VERB_SET_GPIO_DATA,
...
static void alc882_gpio_mute(struct hda_codec *codec, int pin, int muted)
{
	...
	gpiostate = ls_hda_codec_read(codec, codec->afg, 0,
	...
	gpiomask = ls_hda_codec_read(codec, codec->afg, 0,
				      AC_VERB_GET_GPIO_MASK, 0);
	gpiomask |= (1 << pin);

	gpiodir = ls_hda_codec_read(codec, codec->afg, 0,
				     AC_VERB_GET_GPIO_DIRECTION, 0);
	gpiodir |= (1 << pin);


	ls_hda_codec_write(codec, codec->afg, 0,
...
{
	unsigned char bits = ls_hda_jack_detect(codec, 0x18) ? HDA_AMP_MUTE : 0;
...
{
	unsigned int present = ls_hda_jack_detect(codec, 0x1b);
...
{
	unsigned int present = ls_hda_jack_detect(codec, 0x14);
...
static void alc883_clevo_m720_mic_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x18);
	ls_hda_codec_amp_stereo(codec, 0x0b, HDA_INPUT, 1,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
}
...
{
	int bits = ls_hda_jack_detect(codec, 0x14) ? HDA_AMP_MUTE : 0;
...
{
	int bits = ls_hda_jack_detect(codec, 0x1b) ? HDA_AMP_MUTE : 0;
...
static void alc889A_mb31_automute(struct hda_codec *codec)
{
	...
	if (ls_hda_codec_read(codec, 0x15, 0, AC_VERB_GET_CONNECT_SEL, 0)
	    == 0x00) {
		present = ls_hda_jack_detect(codec, 0x15);
		ls_hda_codec_amp_stereo(codec, 0x14,  HDA_OUTPUT, 0,
			HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
		ls_hda_codec_amp_stereo(codec, 0x16, HDA_OUTPUT, 0,
			HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
	}
}
					      ...
					      hda_nid_t dac)
{
	...
	if (dac == 0x25)
		...
		return;
	ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_CONNECT_SEL, idx);
}
...
static void alc882_auto_init_input_src(struct hda_codec *codec)
{
	...
	for (c = 0; c < spec->num_adc_nids; c++) {
		...
		conns = ls_hda_get_connections(codec, nid, conn_list,
	...
	for (i = 0; i < cfg->num_inputs; i++) {
		...
		if (get_wcaps(codec, nid) & AC_WCAP_IN_AMP) {
			...
			snprintf(label, sizeof(label), "%s Boost",
				 ls_hda_get_autocfg_input_label(codec, cfg, i));
...
static int patch_alc882(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC882_MODEL_LAST,
						  alc882_models,
						  alc882_cfg_tbl);

	if (board_config < 0 || board_config >= ALC882_MODEL_LAST)
		board_config = ls_hda_check_board_codec_sid_config(codec,
	...
	if (!spec->adc_nids && spec->input_mux) {
		...
		for (i = 0; i < ARRAY_SIZE(alc882_adc_nids); i++) {
			...
			err = ls_hda_get_connections(codec, nid, &cap, 1);
			if (err < 0)
				continue;
			err = ls_hda_get_connections(codec, cap, items,
						      ARRAY_SIZE(items));
			if (err < 0)
				continue;
...
static void alc262_hp_master_update(struct hda_codec *codec)
{
	...
	/* HP & line-out */
	ls_hda_codec_write_cache(codec, 0x1b, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_HP : 0);
	ls_hda_codec_write_cache(codec, 0x15, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_HP : 0);
	/* mono (speaker) depending on the HP jack sense */
	val = val && !spec->jack_present;
	ls_hda_codec_write_cache(codec, 0x16, 0,
				  AC_VERB_SET_PIN_WIDGET_CONTROL,
				  val ? PIN_OUT : 0);
}
...
static void alc262_hp_bpc_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls_hda_jack_detect(codec, 0x1b);
...
static void alc262_hippo_master_update(struct hda_codec *codec)
{
	...
	ls_hda_codec_amp_stereo(codec, hp_nid, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, mute);
	/* mute internal speaker per jack sense */
	if (spec->jack_present)
		mute = HDA_AMP_MUTE;
	if (line_nid)
		ls_hda_codec_amp_stereo(codec, line_nid, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
	if (speaker_nid && speaker_nid != line_nid)
		ls_hda_codec_amp_stereo(codec, speaker_nid, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
}
...
static void alc262_hippo_automute(struct hda_codec *codec)
{
	...
	spec->jack_present = ls_hda_jack_detect(codec, hp_nid);
	alc262_hippo_master_update(codec);
}
...
static void alc262_fujitsu_automute(struct hda_codec *codec, int force)
{
	...
	if (force || !spec->sense_updated) {
		spec->jack_present = ls_hda_jack_detect(codec, 0x14) ||
				     ls_hda_jack_detect(codec, 0x1b);
	...
	if (spec->jack_present)
		...
		mute = ls_hda_codec_amp_read(codec, 0x14, 0, HDA_OUTPUT, 0);
	ls_hda_codec_amp_stereo(codec, 0x15, HDA_OUTPUT, 0,
				 HDA_AMP_MUTE, mute);
}
...
static void alc262_lenovo_3000_automute(struct hda_codec *codec, int force)
{
	...
	if (spec->jack_present) {
		...
		mute = ls_hda_codec_amp_read(codec, 0x1b, 0, HDA_OUTPUT, 0);
		ls_hda_codec_amp_stereo(codec, 0x14, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
		ls_hda_codec_amp_stereo(codec, 0x16, HDA_OUTPUT, 0,
					 HDA_AMP_MUTE, mute);
				  ...
				  int dir, int idx, long *valp)
{
	...
	for (i = 0; i < 2; i++, valp++)
		change |= ls_hda_codec_amp_update(codec, nid, i, dir, idx,
						   HDA_AMP_MUTE,
						   *valp ? 0 : HDA_AMP_MUTE);
	return change;
}
...
static struct snd_kcontrol_new alc262_fujitsu_mixer[] = {
	HDA_BIND_VOL("Master Playback Volume", &alc262_fujitsu_bind_master_vol),
	{
		...
		.info = ls_hda_mixer_amp_switch_info,
		.get = ls_hda_mixer_amp_switch_get,
...
static int patch_alc262(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC262_MODEL_LAST,
						  alc262_models,
						  alc262_cfg_tbl);
...
static void alc268_aspire_one_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x15);
	bits = present ? HDA_AMP_MUTE : 0;
	ls_hda_codec_amp_stereo(codec, 0x0f, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls_hda_codec_amp_stereo(codec, 0x0f, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
}
...
static void alc268_auto_init_hp_out(struct hda_codec *codec)
{
	...
	if (spec->autocfg.mono_out_pin)
		ls_hda_codec_write(codec, spec->autocfg.mono_out_pin, 0,
...
static void alc268_auto_init_mono_speaker_out(struct hda_codec *codec)
{
	...
	if (line_nid == 0x1d || speaker_nid == 0x1d) {
		ls_hda_codec_write(codec, speaker_nid, 0,
				    AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_OUT);
		/* mute mixer inputs from 0x1d */
		ls_hda_codec_write(codec, 0x0f, 0,
				    AC_VERB_SET_AMP_GAIN_MUTE,
				    AMP_IN_UNMUTE(1));
		ls_hda_codec_write(codec, 0x10, 0,
				    AC_VERB_SET_AMP_GAIN_MUTE,
				    AMP_IN_UNMUTE(1));
	} else {
	...
	ls_hda_codec_write(codec, 0x02, 0,
			    AC_VERB_SET_AMP_GAIN_MUTE, dac_vol1);
	ls_hda_codec_write(codec, 0x03, 0,
...
static int patch_alc268(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC268_MODEL_LAST,
						  alc268_models,
						  alc268_cfg_tbl);
	...
	if (has_beep) {
		...
		if (!lsda_query_amp_caps(codec, 0x1d, HDA_INPUT))
			/* override the amp caps for beep generator */
			ls_hda_override_amp_caps(codec, 0x1d, HDA_INPUT,
	...
	if (!spec->no_analog && !spec->adc_nids && spec->input_mux) {
		...
		for (i = 0; i < spec->num_adc_nids; i++)
			ls_hda_codec_write_cache(codec, alc268_capsrc_nids[i],
...
static void alc269_quanta_fl1_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x15);
	bits = present ? HDA_AMP_MUTE : 0;
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);

	ls_hda_codec_write(codec, 0x20, 0,
...
static void alc269_lifebook_speaker_automute(struct hda_codec *codec)
{
	...
	present |= ls_hda_jack_detect(codec, 0x1a);
...
static void alc269_lifebook_mic_autoswitch(struct hda_codec *codec)
{
	...
	present_laptop	= ls_hda_jack_detect(codec, 0x18);
	present_dock	= ls_hda_jack_detect(codec, 0x1b);

	/* Laptop mic port overrides dock mic port, design decision */
	if (present_dock)
		ls_hda_codec_write(codec, 0x23, 0,
...
static struct hda_verb alc269_init_verbs[] = {
	 ...
	 */
	/* set vol=0 to output mixers */
	{0x02, AC_VERB_SET_AMP_GAIN_MUTE, 0x7f},
	{0x03, AC_VERB_SET_AMP_GAIN_MUTE, 0x7f},
	...
	{0x15, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_HP},
	{0x16, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_OUT},
	/* Mic1 pin: input vref at 80% */
	{0x18, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_VREF80},
	{0x18, AC_VERB_SET_AMP_GAIN_MUTE, AMP_OUT_MUTE},
	/* Mic2 pin: input vref at 80% */
	{0x19, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_VREF80},
	{0x19, AC_VERB_SET_AMP_GAIN_MUTE, AMP_OUT_MUTE},
	{0x1a, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_IN},
	{0x1b, AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_HP},
...
static int alc269_mic2_mute_check_ps(struct hda_codec *codec, hda_nid_t nid)
{
	...
	if (nid == 0x01 || nid == 0x14) {
		int pinval;
		if (ls_hda_codec_amp_read(codec, 0x14, 0, HDA_OUTPUT, 0) &
		    HDA_AMP_MUTE)
			pinval = 0x24;
		else
			pinval = 0x20;
		/* mic2 vref pin is used for mute LED control */
		ls_hda_codec_update_cache(codec, 0x19, 0,
					   AC_VERB_SET_PIN_WIDGET_CONTROL,
					   pinval);
	}
	return alc_check_power_status(codec, nid);
}
#endif /* CONFIG_LS7A_HDA_POWER_SAVE */
...
static int patch_alc269(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC269_MODEL_LAST,
						  alc269_models,
						  alc269_cfg_tbl);
...
{
	unsigned int present = ls_hda_jack_detect(codec, 0x0f);

	ls_hda_codec_amp_stereo(codec, 0x16, HDA_INPUT, 0,
				 HDA_AMP_MUTE, present ? HDA_AMP_MUTE : 0);
	ls_hda_codec_amp_stereo(codec, 0x1a, HDA_INPUT, 3,
...
static hda_nid_t alc861_look_for_dac(struct hda_codec *codec, hda_nid_t pin)
{
	...
	if (ls_hda_get_connections(codec, pin, &mix, 1) != 1)
		return 0;
	num = ls_hda_get_connections(codec, mix, srcs, ARRAY_SIZE(srcs));
	if (num < 0)
		return 0;
	for (i = 0; i < num; i++) {
		unsigned int type;
					      ...
					      int pin_type, hda_nid_t dac)
{
	...
	ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_PIN_WIDGET_CONTROL,
			    pin_type);
	ls_hda_codec_write(codec, dac, 0, AC_VERB_SET_AMP_GAIN_MUTE,
			    AMP_OUT_UNMUTE);
	if (ls_hda_get_connections(codec, nid, &mix, 1) != 1)
		return;
	num = ls_hda_get_connections(codec, mix, srcs, ARRAY_SIZE(srcs));
	if (num < 0)
		return;
	for (i = 0; i < num; i++) {
		...
		if (srcs[i] == dac || srcs[i] == 0x15)
			...
			mute = AMP_IN_MUTE(i);
		ls_hda_codec_write(codec, mix, 0, AC_VERB_SET_AMP_GAIN_MUTE,
				    mute);
	}
}
...
static int patch_alc861(struct hda_codec *codec)
{
	...
	codec->spec = spec;
        ...
        board_config = ls_hda_check_board_config(codec, ALC861_MODEL_LAST,
						  alc861_models,
						  alc861_cfg_tbl);
	...
	err = ls_hda_attach_beep_device(codec, 0x23);
	if (err < 0) {
		alc_free(codec);
		return err;
	}
...
static int patch_alc861vd(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC861VD_MODEL_LAST,
						  alc861vd_models,
						  alc861vd_cfg_tbl);
...
static void alc662_lenovo_101e_ispeaker_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x14);
	bits = present ? HDA_AMP_MUTE : 0;
...
static void alc663_m51va_speaker_automute(struct hda_codec *codec)
{
	...
	present = ls_hda_jack_detect(codec, 0x21);
	bits = present ? HDA_AMP_MUTE : 0;
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
...
static void alc663_21jd_two_speaker_automute(struct hda_codec *codec)
{
	...
	ls_hda_codec_amp_stereo(codec, 0x0c, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
	ls_hda_codec_amp_stereo(codec, 0x0e, HDA_INPUT, 0,
				 HDA_AMP_MUTE, bits);
	ls_hda_codec_amp_stereo(codec, 0x0e, HDA_INPUT, 1,
				 HDA_AMP_MUTE, bits);
}
...
static void alc662_f5z_speaker_automute(struct hda_codec *codec)
{
	...
	ls_hda_codec_write(codec, 0x14, 0,
...
static void alc663_two_hp_m1_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls_hda_jack_detect(codec, 0x21);
	present2 = ls_hda_jack_detect(codec, 0x15);

	if (present1 || present2) {
		ls_hda_codec_write_cache(codec, 0x14, 0,
...
static void alc663_two_hp_m2_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls_hda_jack_detect(codec, 0x1b);
	present2 = ls_hda_jack_detect(codec, 0x15);
...
static void alc663_two_hp_m7_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls_hda_codec_read(codec, 0x1b, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	present2 = ls_hda_codec_read(codec, 0x21, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	...
	if (present1 || present2) {
		ls_hda_codec_write_cache(codec, 0x14, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, 0);
		ls_hda_codec_write_cache(codec, 0x17, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, 0);
	} else {
		ls_hda_codec_write_cache(codec, 0x14, 0,
			AC_VERB_SET_PIN_WIDGET_CONTROL, PIN_OUT);
...
static void alc663_two_hp_m8_speaker_automute(struct hda_codec *codec)
{
	...
	present1 = ls_hda_codec_read(codec, 0x21, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
	present2 = ls_hda_codec_read(codec, 0x15, 0,
			AC_VERB_GET_PIN_SENSE, 0)
			& AC_PINSENSE_PRESENCE;
				   ...
				   hda_nid_t dac)
{
	...
	num = ls_hda_get_connections(codec, pin, mix, ARRAY_SIZE(mix));
	for (i = 0; i < num; i++) {
...
static hda_nid_t alc662_look_for_dac(struct hda_codec *codec, hda_nid_t pin)
{
	...
	num = ls_hda_get_connections(codec, pin, srcs, ARRAY_SIZE(srcs));
	if (num < 0)
		return 0;
	for (i = 0; i < num; i++) {
					      ...
					      hda_nid_t dac)
{
	...
	alc_set_pin_output(codec, nid, pin_type);
	/* need the manual connection? */
	num = ls_hda_get_connections(codec, nid, srcs, ARRAY_SIZE(srcs));
	if (num <= 1)
		return;
	for (i = 0; i < num; i++) {
		if (alc662_mix_to_dac(srcs[i]) != dac)
			continue;
		ls_hda_codec_write(codec, nid, 0, AC_VERB_SET_CONNECT_SEL, i);
		return;
	}
}
...
static int patch_alc662(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC662_MODEL_LAST,
						  alc662_models,
				                  alc662_cfg_tbl);
	if (board_config < 0) {
		printk(KERN_INFO "hda_codec: %s: BIOS auto-probing.\n",
...
static void alc680_rec_autoswitch(struct hda_codec *codec)
{
	...
	for (i = 0; i < cfg->num_inputs; i++) {
		nid = cfg->inputs[i].pin;
		if (!(ls_hda_query_pin_caps(codec, nid) &
	...
	nid = 0x07;
	if (pin_found)
		ls_hda_get_connections(codec, pin_found, &nid, 1);

	if (nid != spec->cur_adc)
		__ls_hda_codec_cleanup_stream(codec, spec->cur_adc, 1);
	spec->cur_adc = nid;
	ls_hda_codec_setup_stream(codec, nid, spec->cur_adc_stream_tag, 0,
				   spec->cur_adc_format);
}
...
static int alc680_capture_pcm_cleanup(struct hda_pcm_stream *hinfo,
				      ...
				      struct snd_pcm_substream *substream)
{
	ls_hda_codec_cleanup_stream(codec, 0x07);
	ls_hda_codec_cleanup_stream(codec, 0x08);
	ls_hda_codec_cleanup_stream(codec, 0x09);
	return 0;
}
...
static int patch_alc680(struct hda_codec *codec)
{
	...
	board_config = ls_hda_check_board_config(codec, ALC680_MODEL_LAST,
						  alc680_models,
						  alc680_cfg_tbl);
 ...
 * patch entries
 */
static struct hda_codec_preset ls_hda_preset_realtek[] = {
